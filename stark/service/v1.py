...
def get_choice_text(title, field):
    """
    对于Stark组件中定义列是，choice如果想要显示中文信息，调用此方法即可
...
class SearchGroupRow(object):
    ...
    def __iter__(self):
        ...
        for item in self.queryset_or_tuple:
            ...
            query_dict._mutable = True          # query_dict 默认是不能修改的，True就可以被修改
            ...
            if not self.option.is_multi:        # 单选
                ...
                muti_val_list = query_dict.getlist(self.option.field)
                if value in origin_value_list:
                    muti_val_list.remove(value)     # 已经选，把自己去掉
                    query_dict.setlist(self.option.field, muti_val_list)
                    yield '<a class="active" href="?%s">%s</a>' % (query_dict.urlencode(), text)
                else:
                    muti_val_list.append(value)
                    query_dict.setlist(self.option.field, muti_val_list)
                    yield '<a href="?%s">%s</a>' % (query_dict.urlencode(), text)
...
class Option(object):
    def __init__(self, field, is_multi=False, db_condition=None, text_func=None, value_func=None):
        ...
        :param text_func: 定义一个函数，用于显示组合搜索按钮页面文本
    ...
    def get_queryset_or_tuple(self, model_class, request, *args, **kwargs):
        ...
        if isinstance(field_object, ForeignKey) or isinstance(field_object, ManyToManyField):
            ...
            queryset_data = field_object.related_model.objects.filter(**db_condition)
            return SearchGroupRow(title, queryset_data, self, request.GET)
        else:
            # 获取choice中的数据
            self.is_choice = True
            choices_data = field_object.choices
            return SearchGroupRow(title, choices_data, self, request.GET)
...
class StarkHandler(object):
    ...
    def __init__(self, site, model_class, prev):
        ...
        self.model_class = model_class  # 表类的对象
    ...
    def get_search_group_condition(self, request):
        ...
        for option in self.get_search_group():
            if option.is_multi:     # 多选
                values_list = request.GET.getlist(option.field)     # 使用getlist支持多选 tags=[1,2]
                if not values_list:
                    continue
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            if action_func_name and action_func_name in action_dict:
                ...
                action_response = func(request, *args, **kwargs)
                if action_response:         # 有返回值
        ...
        for option_object in search_group:
            ...
            print(str(row), type(row))
            search_group_list.append(row)
        print(search_group_list, "?")
        return render(
            request,
    ...
    def get_urls(self):
        patterns = [
            ...
            url('^add/$', self.wrapper(self.add_view), name=self.get_add_url_name),
            url('^change/(?P<pk>\d+)/$', self.wrapper(self.change_view), name=self.get_change_url_name),
            url('^delete/(?P<pk>\d+)/$', self.wrapper(self.delete_view), name=self.get_delete_url_name)
        ]
...
class StarkSite(object):
    ...
    def register(self, model_class, handler_class=None, prev=None):
        ...
        :param model_class: models数据库相关的类
        :param handler_class: 处理请求的视图函数所在的类
        :return:
        """
        if not handler_class:
            handler_class = StarkHandler
        obj_dict = {'model_class': model_class, 'handler': handler_class(self, model_class, prev), 'prev': prev}
        self._registry.append(obj_dict)
    ...
    def get_urls(self):
        ...
        for item in self._registry:
            model_class = item['model_class']       # 获取类
            ...
            if prev:
                patterns.append(url('^%s/%s/%s/' % (app_label, model_name, prev), (handler.get_urls(), None, None)))
            else:
                patterns.append(url('^%s/%s/' % (app_label, model_name), (handler.get_urls(), None, None)))
        return patterns
